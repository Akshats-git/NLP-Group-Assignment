...
- All vocabulary/model lookups are case-INSENSITIVE: every token is lowercased
  ...
  not tracked here — restoring casing on output is a Phase 6 (CLI) concern,
  not a language-model concern.
...
def clean_sentence(tokens: Iterable[str]) -> List[str]:
    ...
    Non-word tokens (punctuation, numbers) are dropped, not replaced —
    this changes sentence length, which is fine for LM training but
